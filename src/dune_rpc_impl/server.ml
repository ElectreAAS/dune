...
let handler (t : _ t Fdecl.t) handle : 'build_arg Dune_rpc_server.Handler.t =
  ...
  let () =
    let f _ () =
      ...
      | Failure ->
        ...
        in
        assert (Option.is_none missing_msg && List.is_empty missing);
        ()
    in
  ...
  let () =
    let f _ files =
      ...
      | _, [] -> Fiber.return Dune_rpc.Build_outcome_with_diagnostics.Success
      | None, _ -> failwith "no missing msg???"
      | Some missing_msg, missing ->
        let errors =
          List.map missing ~f:(fun fn ->
            Dune_rpc.Compound_user_error.make
              ...
              ~related:[])
        in
        Fiber.return (Dune_rpc.Build_outcome_with_diagnostics.Failure errors)
    in
